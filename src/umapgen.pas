...
begin
  ...
  begin
    ...
    repeat
      while Lines[Lo].Length < Mid do
        Inc(Lo);
      while Lines[Hi].Length > Mid do
        Dec(Hi);
      if Lo <= Hi then
      begin
        T := Lines[Lo];
...
begin
  ...
  begin
    if (I < MaxLine) and Lines[I].Active then
      continue;
    //also, add some more links
    if random < ADD_LINKS then
      continue;
...
begin
  ...
  begin
    ...
    begin
      ntries := 1000;
      while (ntries > 0) and not step(RandomNeighbour(ants[i]), i) do
        Dec(ntries);
    end;
  end;
  nsteps := 0;
...
var
  ...
  begin
    if First >= length(Map.Systems) then
      exit;
    if second >= length(Map.Systems) then
      exit;
...
begin
  ...
  for i := n - 2 downto 0 do
    for j := n - 1 downto i + 1 do
      if Distance(Map.Systems[i].X, Map.Systems[i].Y, Map.Systems[j].X,
        Map.Systems[j].Y) < CLOSE_DIST then
      begin
        Map.Systems[j].Free;
        if j <> n - 1 then
        begin
