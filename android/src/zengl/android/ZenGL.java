...
public class ZenGL extends GLSurfaceView {
	...
	public ZenGL(Context context, String appName, String appSourceDir) {
		...
		/*
		 * System.loadLibrary( "zenjpeg" ); System.loadLibrary( "openal" );
		 * System.loadLibrary( "ogg" ); System.loadLibrary( "vorbis" );
		 * System.loadLibrary( "theoradec" ); System.loadLibrary( "chipmunk" );
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			for (int i = 0; i < count; i++) {
				...
				zglNativeTouch(pointerID, event.getX(i), event.getY(i),
						event.getPressure(i));
			}
			break;
		}
		...
		case MotionEvent.ACTION_POINTER_DOWN: {
			...
			if (pointerID >= 0 && pointerID < event.getPointerCount())
				zglNativeTouch(pointerIndex, event.getX(pointerID),
						event.getY(pointerID), event.getPressure(pointerID));
			break;
		}
	...
	public void SwapBuffers() {
		try {
			...
			if (currDisplay == EGL10.EGL_NO_DISPLAY)
				return;

			EGLSurface currSurface = currEGL
					.eglGetCurrentSurface(EGL10.EGL_DRAW);
			if (currSurface == EGL10.EGL_NO_SURFACE)
				return; 
